...
import com.hossein.mywallet.databinding.ItemUserBinding
...
class UsersAdapter : RecyclerView.Adapter<UsersAdapter.ViewHolder>() {
    ...
    inner class ViewHolder : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item: UserEntity) {
            ...
            binding.apply {
                //Set text
              //  itemUserInfo.text = "${item.userId} - ${item.Babat} ${item.Price}"
                itemUserInfo.text = " مبلغ: ${item.Price} بابت: ${item.Babat}  "
                //Click
